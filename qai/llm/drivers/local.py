...
class LocalDriver(BaseDriver):
    ...
    def ask(self, prompt, context=""):
        ...
        # =========================================================
        # HARD RAG QUALITY GATE
        # =========================================================
        # Never allow tiny/incomplete retrieval fragments to guide
        # the local LLM. A 35-character fragment is not sufficient
        # evidence for answering a factual question.
        ...
        # ---------------------------------------------------------
        # FILTER WEAK / TRIVIAL RAG DOCUMENTS
        # ---------------------------------------------------------
        # Very short documents are usually incomplete metadata,
        # titles, fragments, or bad retrieval noise.
        # They must not be allowed to steer local generation.
        filtered_documents = []
        ...
        for doc in documents:
            ...
            if len(doc_text) < 100:
                print(
                    "[LocalDriver] Ignoring weak RAG document:",
                    len(doc_text),
                    "chars"
                )
                continue
        ...
        # ============================================================
        # APPROVED QAI KNOWLEDGE DIRECT ANSWER
        # ============================================================
        # If the knowledge item was explicitly approved by the
        # supervisor, NEVER let the local LLM reinterpret or invent
        # an answer from it.
        _approved_qai_docs = []
        ...
        for _qai_doc in documents:
            ...
            _qai_teacher = str(
                _qai_doc.get("teacher", "")
                or ""
            ).strip().lower()

            _qai_confidence = _qai_doc.get("confidence", 0)

            try:
                _qai_confidence = float(_qai_confidence)
            except Exception:
                _qai_confidence = 0.0
            ...
            if (
                ...
                and _qai_approved
                and _qai_teacher == "supervisor"
                and _qai_confidence >= 1.0
                and _qai_text
            ):
                _approved_qai_docs.append(_qai_doc)

        if _approved_qai_docs:
            _approved_qai_docs.sort(
                key=lambda d: float(d.get("final_score", d.get("score", 0)) or 0),
                reverse=True,
            )

            _approved_doc = _approved_qai_docs[0]
            _approved_answer = str(
                _approved_doc.get("text")
                or _approved_doc.get("content")
                or _approved_doc.get("knowledge")
                or ""
            ).strip()
            ...
            return {
                ...
                "source": "qai_learning",
                "confidence": 1.0,
                "relevance": _approved_doc.get("relevance", 100),
